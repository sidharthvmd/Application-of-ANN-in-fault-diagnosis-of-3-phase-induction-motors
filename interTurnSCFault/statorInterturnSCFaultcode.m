...
load_system('statorInterturnSCFault.slx');
...
targetValues = zeros(numSamples*18, 2);  % Multi-class target values: 5 for stator interturn sc

% Set initial parameter values
initialValue = 220;  %reconsider
finalValue = 240;
...
for i = 1:numSamples
    ...
    set_param(['statorInterturnSCFault','/', 'VoltageSource1'], 'Amplitude', num2str(voltageValue));  %fault ratio and fault resistance 
    set_param(['statorInterturnSCFault','/','VoltageSource2'], 'Amplitude', num2str(voltageValue));
    set_param(['statorInterturnSCFault','/','VoltageSource3'], 'Amplitude', num2str(voltageValue));

    % Simulate the model
    sim("statorInterturnSCFault.slx");
...
save('ann_dataset_statorInterturnSCFault.mat', 'inputFeatures', 'targetValues');
writematrix([inputFeaturesAll, targetValuesAll], 'ann_dataset_statorInterturnSCFault.csv');