...
hiddenLayerSize = 1; % Two hidden layers, each containing three neurons
net = patternnet(hiddenLayerSize);

% Set activation function to hyperbolic tangent for hidden layers
for j = 1:length(hiddenLayerSize)
    net.layers{j}.transferFcn = 'purelin'; % Hyperbolic tangent activation function
end
...
net.performParam.regularization = 0.01; % Adjust regularization strength as needed
x_train = x(trainInd, :);
y_train = yt(:, trainInd);
